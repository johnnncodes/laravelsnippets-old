...
<table id="properties-table" class="table table-striped table-hover">
	<thead>
      	<tr>
          	...
          	<th width="25%"><span style="float:right; padding-right: 140px;">Actions</span></th>                                           
      </tr>
  	</thead>   
	<tbody>
		...
		@foreach($snippets->results as $snippet)
		    ...
		    <tr>
		        ...
		        <td class="center"><a class="title-link" href="{{ action('admin.snippets@index', array($snippet->id)) }}">{{ e($snippet->title) }}</a></td>
		        <td class="center" style="text-align:right;">
		        	<?php if($snippet->published != '1'): ?>
					   	<a href="{{ action('admin.snippets@publish', array($snippet->id)) }}" class="btn btn-info">
			        		Publish
			       		</a>
					<?php endif; ?>		        	
					<a href="{{ action('admin.snippets@index', array($snippet->id)) }}" class="btn btn-info">
		        		View
		        	</a>
		        	<a href="{{ action('admin.snippets@edit', array($snippet->id)) }}" class="btn btn-info">
		        		Edit
		        	</a>
		            ...
		            <div class="modal hide" id="myModal-delete-{{ $snippet->id }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
						...
						<div class="modal-footer">
						   ...
						   <a href="{{ action('admin.snippets@delete', array($snippet->id)) }}" class="btn btn-danger">Confirm Delete</a> 
						</div>
